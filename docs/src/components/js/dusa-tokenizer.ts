...
export function dusaTokenizer(
  predicates: string[],
  builtins: string[],
): StreamParser<ParserState, Token> {
  const pred = new Set(predicates);
  const blt = new Set(builtins);
  return {
        ...
        case 'Normal':
          ...
          if ((tok = stream.eat(META_ID_TOKEN) ?? stream.eat(META_NUM_TOKEN))) {
            ...
            if (tok.match(CONST_TOKEN)) {
              return {
                state,
                tag: blt.has(tok) ? 'builtin' : pred.has(tok) ? 'predicate' : 'literal',
