...
potential criminals. Harold's innocence is tied to others: if Amy is innocent, then
...
forces Harold to be guilty, this rule won't contradict that other conclusion.
...
#forbid a is false, c is false, d is false.
...
<Dusa predicates={['player', 'throw']} code={`
...
<Dusa predicates={['player', 'throw', 'outcome', 'tie', 'winner']} code={`
