...
After the `:-` symbol come the **premises**. (There can be more than one premise. If there
...
<Dusa predicates={['start', 'reachable', 'edge']} code={`
...
<Dusa predicates={['color', 'node']} code={`
...
<Dusa predicates={['edge', 'node']} code={`
...
<Dusa predicates={['edge', 'color']} code={`
...
<Dusa predicates={['representative', 'node']} code={`
...
<Dusa predicates={['representative', 'edge']} code={`
